/* ==========================================================================
   ...
   规则：
     ...
     · 同一场比赛不可重复串联（竞彩规则）；
     · 组合赔率 = 各腿腿赔率相乘（腿赔率 = 同场同玩法的多个结果取最高 SP）；
       联合概率 = 各腿腿概率相乘（腿概率 = 同场同玩法各结果概率相加 · 独立性近似）；
...
(function (global) {
  ...
  function choicesFor(g, cfg, mainUsed) {
    ...
    function pack(play, takeAll, isMain) {
      ...
      /* 一条腿 = 一场比赛的一个玩法，腿内多结果是「覆盖」关系：
         腿赔率 = 腿内各结果 SP 的**最大值**（取最高的一档计价，不做相乘）；
         腿概率 = 腿内各结果模型概率**相加**（任选其一命中即算这条腿命中）。 */
      var sp = 0, p = 0;
  ...
  function head(cfg) {
    return '<div class="pagehead"><div class="wrap">' +
      ...
      '同一场比赛不可重复串联（竞彩规则），' +
      '联合概率为各腿腿概率相乘的独立性近似（腿内多结果按概率相加）。<b>模拟结果为结构参考，不是盈利承诺。</b></div>' +
      '</div></div>';
  }
    ...
    if (groups.length < k) {
      ...
      body = '<div class="card-b flush"><div class="scrollx"><table class="tbl tbl-dense">' +
        ...
        '<div class="card-b" style="border-top:1px solid var(--line)">' +
          ...
          '组合赔率 = 各腿赔率相乘；联合概率 = 各腿腿概率相乘，其中腿概率 = 同场同玩法各结果概率<b>相加</b>（任选其一命中即算该腿命中，独立性近似）。' +
  ...
  function drawSimPNG(S) {
    ...
    ctx.fillText('同一场同玩法的多个结果合并为一条腿：腿赔率取腿内最高 SP，腿概率取腿内各结果概率相加（任选其一命中即算该腿命中）；组合赔率 = 各腿腿赔率相乘，联合概率 = 各腿腿概率相乘（独立性近似；半全场为官方赔率隐含概率口径）；EV = 联合概率 × 组合赔率 − 1。模拟为结构参考，不构成任何盈利承诺。', PAD, 180);
  ...
  /* 测试/调试用：取最近一次计算结果（tools/sim-compose-test.js 用它精确校验
     腿赔率=最高 SP、腿概率=各结果概率相加 等口径） */
  JX.simLast = function () { return lastSim; };
})(window);
