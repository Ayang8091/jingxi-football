...
| 串关搭配 | `parlay.html` | 独立工作台（数据与主站联动）：候选腿池勾选、自动选号 2~4 腿、当期赔率/投入可编辑锁定、参考期可清除、倍投/固定投入双模式 |
...
- **多通道抓取链**：直连（两次，其中一次补正 Referer）→ 公共 CORS 中继 → 上次成功快照 → 演示数据；
  任一环节失败自动降级，接口不可达时按初版形态安静展示演示数据，不弹错误横幅。
- 接入后自动切换为**实时模式**：λ 由赔率去水后的市场隐含概率反解，全部概率/EV 仍由模型实时计算；缺少 xG / 伤停 / 交锋 / 亚指的字段以占位说明如实降级，不显示假数据。
- 接口不可用时自动回退（优先展示上次成功的实时快照），数据缓存于 localStorage，刷新失败仍可离线浏览。
