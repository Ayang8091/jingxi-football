...
| 串关搭配 | `parlay.html` | 串关组合清单（逐腿标注模型概率/公平赔率/EV）、倍投资金计划、风险测算、数学结论 |
...
页面顶部状态条右侧的 **「数据更新」按钮** 点击即连接中国体育彩票官方接口刷新数据：

```
https://webapi.sporttery.cn/gateway/jc/football/getMatchCalculatorV1.qry
```

- 返回真实竞彩赛程与胜平负 / 让球 / 总进球 / 比分 / 半全场 SP，浏览器直连（官方接口已开启 CORS）。
- 接入后自动切换为**实时模式**：λ 由赔率去水后的市场隐含概率反解，全部概率/EV 仍由模型实时计算；缺少 xG / 伤停 / 交锋 / 亚指的字段以占位说明如实降级，不显示假数据。
- 接口不可用时自动回退演示数据，状态条明确标注当前模式；数据缓存于 localStorage，刷新失败仍可离线浏览上次结果。

### 串关搭配栏目

基于《升级版利润测算表》的**倍投（追投）算法**逐格复刻并与原表校验一致：

```
出票额(n) = (累投(n-1) + 日利润 × n) / (赔率 − 1)    // 向上取偶，与 Excel EVEN 一致
```

- 支持逐期赔率切换与续投变体（起始累投、首期目标 0）。
- 每期命中即覆盖全部累计投入并实现目标利润；未中则进入下一期加倍。
- 风险面板如实量化：全周期不中概率 = q^N、峰值资金占用、期望资金效率、平均全灭周期。
- 串关组合由模型筛选的正 EV 单腿聚合生成，明确标注「历史水位下可能长时间无正 EV 推荐」。
...
│   └── js/
│       ├── data.js           # 数据层：内置演示快照（实时模式下仅作回退）
│       ├── datasource.js     # 数据源：官方接口接入 + 规范化 + λ 反解 + demo/live 状态机
│       ├── parlay.js         # 串关引擎：倍投计划 + 组合构建 + 风险测算
│       ├── core.js           # 引擎：模型计算 + SVG 图表 + 页面骨架
│       ├── app.js            # 首页赛事列表 + 状态条 / 数据更新按钮
│       ├── pages-parlay.js   # 串关搭配页
│       ├── pages-match.js    # 单场剖析页（含真实数据降级块）
│       ├── pages-model.js    # 模型方法论页
│       ├── pages-data.js     # 战绩公示 + 数据看板
│       ├── pages-law.js      # 合规页
│       └── boot.js           # 启动路由（订阅数据源变化重渲染）
└── tools/
    ├── calibrate-demo-data.js  # 演示数据校准脚本
    ├── data.backup.js          # 校准前的数据备份
    ├── deploy-github.sh        # GitHub Pages 一键部署
    ├── fixtures/               # 官方接口真实响应快照（测试夹具）
    └── render-test.js          # jsdom 渲染冒烟测试（7 页面 × demo/live 双模式）
```
